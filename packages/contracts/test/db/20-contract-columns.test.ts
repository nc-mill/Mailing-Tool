...
import { FOREIGN_CONTRACT_COLUMNS, MESSAGES_CONTRACT_COLUMNS } from '../../src/outbox';
import { type ContractDb, packageRoot, startContractDb, stopContractDb } from './helpers';
...
describe('contracts-schema: kontraktní sloupce', () => {
  it('manifest leží tam, kde ho hledá P01, a má tvar, který P01 čte', async () => {
    ...
    // přeskočil; kontraktní sloupce po migracích nehlídalo nic.
  ...
  it('sloupce cizích tabulek v manifestu jsou podmnožinou kontraktního výčtu', async () => {
    ...
    for (const table of ['campaigns', 'workspaces', 'suppressions'] as const) {
      expect(Object.keys(manifest[table]!).sort()).toEqual(
        [...FOREIGN_CONTRACT_COLUMNS[table]!].sort(),
      );
    }
  });
  ...
  it('každý kontraktní sloupec messages existuje a má očekávaný typ', async () => {
    ...
    const actual = Object.fromEntries(
      rows.map((r: { column_name: string; data_type: string }) => [r.column_name, r.data_type]),
    );
    ...
    for (const [column, type] of Object.entries(MESSAGES_CONTRACT_COLUMNS)) {
      ...
      else if (actual[column] !== type)
        mismatches.push(`${column} má typ ${actual[column]}, čeká se ${type}`);
    }
  ...
  it('sender nemá UPDATE grant na created_at a má ho na ambiguous_count', async () => {
    ...
    const granted = rows.map((r: { column_name: string }) => r.column_name);
  ...
  it('politika sender_bypass existuje na každé tabulce, kterou sender čte', async () => {
    ...
    const tables = rows.map((r: { tablename: string }) => r.tablename).sort();
