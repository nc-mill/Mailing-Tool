...
export async function checkDmarc(
  ...
  try {
    ...
    return {
      ...
      pct: null,
      color: 'red',
      findings: [{ code: 'dmarc_missing', severity: 'error' }],
    };
  }
  ...
  if (records.length > 1) {
    return {
      ...
      pct: null,
      color: 'red',
      findings: [{ code: 'dmarc_multiple_records', severity: 'error' }],
    };
  }
  ...
  if (!policy || !['none', 'quarantine', 'reject'].includes(policy)) {
    return {
      ...
      pct: null,
      color: 'red',
      findings: [{ code: 'dmarc_invalid_syntax', severity: 'error', params: { tag: 'p' } }],
    };
  }
