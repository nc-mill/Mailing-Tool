...
-- ===========================================================================
-- Výmaz podle článku 17 musí smazat i HISTORII PROCHÁZENÍ, tedy sloupec `page`.
--
-- CO BYLO ŠPATNĚ. Migrace 0022 rozšířila sloupcový grant na `web_events`
-- o `properties` a `context`, protože to byly sloupce, na které se tehdy
-- ptalo zadání. `page` v ní SCHVÁLNĚ NEBYL: nález se objevil až při ověřování
-- výmazu na reálných datech a rozhodnutí o něm patřilo zadavateli, ne
-- rozšíření mimochodem. Zadavatel ho 6. 8. 2026 potvrdil, tahle migrace ho
-- provádí.
--
-- CO V TOM SLOUPCI JE. Ověřeno dotazem proti běžící databázi, ne odvozeno:
-- klíče `url`, `path`, `search`, `referrer` a `title`. Je to stejná třída
-- údaje jako `context`:
--   * adresa navštívené stránky sama o sobě popisuje chování konkrétního
--     člověka („byl na /kosik", „četl /produkt/…"),
--   * `referrer` říká, odkud přišel, tedy kus téže historie,
--   * `search` bere, co do něj zákazník dá, takže se do řádku dostane i to,
--     co posílá v dotazovém řetězci, včetně e-mailu.
--
-- Zachovat „jen doménu" nejde. `page` je jsonb a částečný přepis by byl
-- allowlist podle jmen klíčů, tedy přesně ta konstrukce, kterou 0022 zamítla
-- u `properties`: slib, který kód nemůže dodržet, protože obsah plní zákazník.
--
-- ---------------------------------------------------------------------------
-- PROČ NOVÁ MIGRACE A NE ÚPRAVA 0022
-- ---------------------------------------------------------------------------
-- 0022 je VYDANÁ, tedy aplikovaná a zapsaná v `drizzle.__drizzle_migrations`.
-- Vydaná migrace se needituje ani o bílý znak: runner hlídá drift hashem
-- souboru a editace skončí na `migration_hash_mismatch` (exit 6). Ověřeno
-- spuštěním, ne odvozeno, právě na tomhle souboru.
--
-- ---------------------------------------------------------------------------
-- PROČ SE CELÁ FUNKCE OPISUJE POTŘETÍ
-- ---------------------------------------------------------------------------
-- Rozhodnutí R25: `mlain_apply_grants()` je ÚPLNÝ popis oprávnění a volá ji
-- `mlain doctor --fix` i obnova ze zálohy. Kdyby nové právo leželo mimo ni,
-- první `SELECT mlain_apply_grants()` po obnově by ho smetl REVOKE ALL v cyklu
-- a výmaz by po obnově ze zálohy tiše přestal mazat historii procházení.
-- Platná je vždy definice z NEJVYŠŠÍ migrace, tedy tahle.
--
-- Proti 0024 se mění JEDINÝ řádek, a to sloupcový GRANT UPDATE na web_events.
--
-- ---------------------------------------------------------------------------
-- VÝSTRAHA, KTEROU PLATÍME ZKUŠENOSTÍ Z 6. 8. 2026
-- ---------------------------------------------------------------------------
-- Tahle funkce se v každé migraci OPISUJE CELÁ. Kdo ji opíše, přebírá
-- odpovědnost za VŠECHNA práva, která v ní stojí, ne jen za to svoje.
--
-- VYNECHÁNÍ KTERÉHOKOLI PRÁVA Z PŘEDCHOZÍ MIGRACE HO TIŠE ODEBERE. Není to
-- teoretická past, stalo se to přesně takhle:
--
--   0022 přidala `page` do sloupcového grantu na `web_events`.
--   0024 opsala funkci ve znění BEZ `page`, protože vznikla dřív.
--   Po doběhnutí obou zůstalo `web_events` bez práva zápisu do `page`.
--
-- Nic při tom neselhalo. `CREATE OR REPLACE FUNCTION` starou definici prostě
-- nahradí, `REVOKE ALL` v cyklu na začátku smete všechno a znovu se rozdá jen
-- to, co je ve funkci NAPSANÉ. Poznalo by se to teprve tím, že by výmaz podle
-- článku 17 skončil na `permission denied for table web_events`, tedy až
-- v okamžiku, kdy o svá data někdo požádá. Tahle migrace to právo vrací.
--
-- PRAVIDLO PRO PŘÍŠTĚ: novou migraci s granty zakládej KOPIÍ funkce z nejvyšší
-- existující migrace, ne z té, kterou máš zrovna otevřenou, a před spuštěním
-- porovnej výčty práv proti ní. Katalogový test v `packages/db/test/grants.test.ts`
-- hlídá granty proti politikám, ale rozdíl mezi dvěma zněními TÉŽE funkce
-- nezachytí, protože obě jsou samy o sobě konzistentní.
-- ===========================================================================
...
BEGIN
  ...
  -- místo na constraint.
  --
  -- ZMĚNA PROTI 0009: `properties` a `context` (migrace 0022), `page`
  -- (migrace 0026). Bez nich odstřihne výmaz podle článku 17 jen vazbu
  -- a e-mail, jméno, otisk prohlížeče i historie procházení zůstanou v řádku
  ...
  GRANT  UPDATE (contact_id, identity_merge_id, erased_at, properties, context, page)
