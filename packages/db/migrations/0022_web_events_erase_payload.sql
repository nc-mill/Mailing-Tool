...
-- ===========================================================================
-- Výmaz podle článku 17 musí umět smazat i OBSAH události z prohlížeče,
-- ne jen vazbu na osobu.
--
-- CO BYLO ŠPATNĚ. Sloupcový grant z migrace 0005 pouští aplikační roli na
-- `web_events` jen tři sloupce (`contact_id`, `identity_merge_id`, `erased_at`).
-- Job `gdpr.sever_links` proto uměl odstřihnout vazbu, ale osobní údaje uvnitř
-- `properties` a `context` nechával ležet, protože na ně NEMĚL PRÁVO ZÁPISU.
-- Nešlo to ani poznat: job by pokus o zápis do `properties` odnesl chybou
-- oprávnění, takže tam nikdo pokus nenapsal a data zůstala navždy.
--
-- CO V TĚCH SLOUPCÍCH JE. Ověřeno dotazem proti běžící databázi, ne odvozeno:
--   * `properties` u události `identify` nese `traits` s e-mailem a jménem,
--     dál `external_id` (identifikátor osoby v systému zákazníka) a `signature`
--     (HMAC nad payloadem, tedy taky identifikátor). Zbytek klíčů si definuje
--     zákazník ve vlastním volání SDK, takže obsah NEMÁ SCHÉMA a jakýkoli klíč
--     může nést cokoli.
--   * `context` je proti tomu uzavřená sada (typ `EventContext`) a obsahuje
--     `ip` a `country`, když je provozovatel i projekt zapnou, plus `os`,
--     `browser`, `locale`, `timezone`, `screen`, `viewport` a `device`. IP je
--     osobní údaj z definice, zbytek je učebnicový otisk prohlížeče: každá
--     položka zvlášť je neškodná, dohromady je to identifikace.
--
-- Grant se proto rozšiřuje právě o tyhle dva sloupce a o nic víc. `page` v něm
-- SCHVÁLNĚ NENÍ: nese historii procházení (url, referrer, title) a je to stejná
-- třída údaje, ale patří do vlastního rozhodnutí, ne do rozšíření mimochodem.
--
-- ---------------------------------------------------------------------------
...
-- a výmaz by po obnově ze zálohy tiše přestal mazat obsah událostí. Vydaná
-- migrace se přitom needituje ani o bílý znak (`migration_hash_mismatch`),
-- takže 0005 i 0009 zůstávají historickým záznamem. Platná je vždy definice
-- z NEJVYŠŠÍ migrace.
--
-- Proti 0009 se mění JEDINÝ řádek, a to sloupcový GRANT UPDATE na web_events.
-- ===========================================================================
