...
func BuildMIME(h *campaign.Header, msg outbox.Message, r *Rendered, opts MIMEOptions) ([]byte, error) {
	...
	unsub := []string{}
	// Druhá pojistka k té v Renderer.unsubscribe. Mailto adresa přichází
	// z konfigurace, ne z renderu, takže by se transakční zprávě přilepila
	// i s prázdným UnsubscribeURL a hlavička by vznikla.
	precedenceBulk := opts.PrecedenceBulk
	if r.NoUnsubscribe {
		// Transakční zpráva hromadná není, Precedence: bulk by ji shodila
		// do kategorie propagace a některé servery na ni reagují zpožděním.
	...
	in := mimebuild.Input{
		...
		OneClick:        r.OneClick,
		PrecedenceBulk:  precedenceBulk,
		TestHeader:      r.IsTest,
		AutoGenerated:   r.NoUnsubscribe,
